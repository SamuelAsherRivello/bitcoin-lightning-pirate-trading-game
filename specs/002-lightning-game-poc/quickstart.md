...
1. Confirm the navigation shows `Home`, `Set Up`, `Play Game`, and `Debug Network` from left to right.
2. Confirm `Home` and `Set Up` are enabled before setup is verified, while `Play Game` and `Debug Network` are visible but disabled.
3. Open `Home`.
4. Read `Why this demo exists` and confirm the regtest-only and lab-control warnings are visible.
5. Confirm `Home` includes:
   - A `What is Bitcoin?` summary with an external learn-more link.
   - A `What is Lightning?` summary with an external learn-more link.
...
6. Read the operations table and confirm it includes:
...
11. Refresh the page.
12. Confirm `Play Game` and `Debug Network` are enabled.
...
5. Click `Wait for Next Block`.
6. Confirm the app explains that regtest mined the next block instantly.
...
## 5. Verify Debug Network

1. Open `Debug Network`.
