...
- **FR-006a**: The `Create Nodes` step MUST find or create all Polar nodes required by later steps: the Bitcoin backend, Game Treasury LND node, Taproot Assets node, two NPC LND nodes, and one player LND node.
- **FR-006b**: The `Create Nodes` step MUST request creation before checking readiness, poll all required nodes for started/ready status, and restart the Polar network once if repeated readiness retries show unstable or not-started nodes.
