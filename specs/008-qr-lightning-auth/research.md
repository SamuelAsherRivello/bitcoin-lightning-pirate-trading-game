...
The Polar node workflow itself should start at `Bridge URL`, then `Server Name`, then `Create Nodes`. This reflects the newer stability workaround: create or find all required Polar nodes before funding, minting, or balancing. `Create Nodes` owns the Bitcoin backend, Game Treasury LND node, `GAME_TAPROOT` Taproot Assets node, one player LND node, and two NPC LND nodes. It requests creation, restarts after node topology changes, polls for started/ready status, retries readiness checks, and restarts the Polar network once when repeated readiness retries indicate unstable node startup. Later steps operate on already-created nodes.
