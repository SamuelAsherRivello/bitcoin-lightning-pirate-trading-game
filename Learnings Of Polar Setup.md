...
- Third real bridge-backed run reused the default saved server and stopped on a Docker/Polar port conflict from an orphan `polar-n12-alice` container. The runner now always uses a fresh generated server name, not the visible default input.
